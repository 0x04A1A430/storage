...
cleanup_s3_old_backups() {
    ...
    file_list=$(AWS_ACCESS_KEY_ID="$S3_ACCESS_KEY" \
                ...
                $s3_endpoint_arg 2>/dev/null | grep "backup_.*\.tar\.gz")
...
create_backup() {
    ...
    BACKUP_FILE_FINAL="backup_${backup_scope_tag}_${TIMESTAMP}.tar.gz"
...
METAEOF
    ...
    local caption_text=$'*Размер:* '"${backup_size}"$'\n*Дата:* '"${DATE}"
    ...
    if [[ -f "$BACKUP_DIR/$BACKUP_FILE_FINAL" ]]; then
        if [[ "$UPLOAD_METHOD" == "telegram" ]]; then
            ...
            if send_google_drive_document "$BACKUP_DIR/$BACKUP_FILE_FINAL"; then
                ...
                local tg_success_message=$'*Размер:* '"${backup_size}"$'\n*Дата:* '"${DATE}"
    ...
    if [[ "$RETAIN_BACKUPS_UNIT" == "hours" ]]; then
        find "$BACKUP_DIR" -maxdepth 1 -name "backup_*.tar.gz" -mmin +$((RETAIN_BACKUPS_VALUE * 60)) -delete
    else
        find "$BACKUP_DIR" -maxdepth 1 -name "backup_*.tar.gz" -mmin +$((RETAIN_BACKUPS_VALUE * 24 * 60)) -delete
    fi
    ...
    case "$source_choice" in
        ...
        2)
            ...
            s3_file_list=$(AWS_ACCESS_KEY_ID="$rs_s3_access" \
                ...
                $s3_endpoint_arg 2>/dev/null | grep "backup_.*\.tar\.gz" | sort -r)
